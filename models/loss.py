...
class MultiScaleMelSpectrogramLoss(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        for n_mels, fmin, fmax, s in zip(
            ...
            # 强制 FP32 避免 AMP 导致的 NaN
            with torch.cuda.amp.autocast(enabled=False):
                x_mels = self.mel_spectrogram(x.float(), **kwargs)
                y_mels = self.mel_spectrogram(y.float(), **kwargs)
                # 增加 eps 从 clamp_eps 到 1e-8，避免 log(0) = -inf 在 FP16 中
                x_logmels = torch.log(
                    x_mels.clamp(min=max(self.clamp_eps, 1e-8)).pow(self.pow) + 1e-8
                ) / torch.log(torch.tensor(10.0))
                y_logmels = torch.log(
                    y_mels.clamp(min=max(self.clamp_eps, 1e-8)).pow(self.pow) + 1e-8
                ) / torch.log(torch.tensor(10.0))
