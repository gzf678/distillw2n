...
# 配置路径
whisper_dir = '/mnt/data/share/WHSP_LGU/parallel_simple/whsp/'  # whisper音频目录（输入，耳语）
normal_dir = '/mnt/data/share/WHSP_LGU/parallel_simple/norm/'  # normal音频目录（用于提取说话人嵌入，正常语音）
output_dir = '/mnt/workspace/guanzifan/eval_tmp/distill_w2n_lgu_not/'  # 输出音频目录
...
def parse_filename(filename):
    """
    解析文件名，提取说话人ID和句子ID
    例如：user004_001_whsp.wav -> ('user004', '001')
          user004_001_norm.wav -> ('user004', '001')
    """
    # 匹配格式：user{3位数字}_{3位数字}_{whsp|norm}.wav
    # 例如：user004_001_whsp.wav
    pattern = r'(user\d{3})_(\d{3})_(whsp|norm)\.wav'
    match = re.search(pattern, filename, re.IGNORECASE)
    if match:
        speaker_id = match.group(1)  # user004
        sentence_id = match.group(2)  # 001
        return speaker_id, sentence_id
    return None, None

def build_normal_index():
    """建立normal文件的索引：{speaker_id: {sentence_id: file_path}}"""
    normal_files_wav = list(normal_path.rglob("*.wav"))
    normal_files_WAV = list(normal_path.rglob("*.WAV"))
    normal_files = list(set(normal_files_wav + normal_files_WAV))
    
    for normal_file in normal_files:
        speaker_id, sentence_id = parse_filename(normal_file.name)
        if speaker_id and sentence_id:
            if speaker_id not in speaker_normal_index:
                speaker_normal_index[speaker_id] = {}
            speaker_normal_index[speaker_id][sentence_id] = normal_file
    
    print(f"Indexed normal files for {len(speaker_normal_index)} speakers")
    for speaker_id, sentences in speaker_normal_index.items():
        print(f"  {speaker_id}: {len(sentences)} normal files")

def get_different_sentence_normal_file(whisper_file_path):
    """
    根据whisper文件路径，找到同一说话人但不同句子的normal文件
    例如：user004_001_whsp.wav -> user004_002_norm.wav (同一说话人user004，但不同句子001 vs 002)
    """
    whisper_filename = whisper_file_path.name
    speaker_id, sentence_id = parse_filename(whisper_filename)
    ...
    # 找到不同句子的normal文件（排除相同句子ID）
    available_sentences = [sid for sid in speaker_normals.keys() if sid != sentence_id]
    
    if len(available_sentences) == 0:
        return None
    
    # 使用第一个找到的不同句子的normal文件
    selected_sentence_id = available_sentences[0]
    return speaker_normals[selected_sentence_id]
