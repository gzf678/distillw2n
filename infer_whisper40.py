...
# 配置路径
whisper_dir = '/mnt/data/share/whisper40/whsp/enroll/'  # whisper音频目录（输入）
normal_dir = '/mnt/data/share/whisper40/nature/enroll/'  # normal音频目录（用于提取说话人嵌入）
output_dir = '/mnt/workspace/guanzifan/eval_tmp/distill_w2n_whisper40'  # 输出音频目录
...
def get_speaker_normal_file(whisper_file_path):
    ...
    例如：zyl40_40_whsp.wav -> zyl40_40_norm.wav
    规则：将文件名中的 'whsp' 替换为 'norm'
    """
    # 获取相对于whisper_dir的路径
    rel_path = whisper_file_path.relative_to(whisper_path)
    
    # 获取文件名（如 zyl40_40_whsp.wav）
    filename = rel_path.name
    
    # 获取文件扩展名（保持原始大小写）
    if filename.lower().endswith('.wav'):
        ext = filename[-4:]  # 保留原始大小写的扩展名
        base_name = filename[:-4]
    else:
        ext = ''
        base_name = filename
    
    # 将文件名中的 'whsp' 替换为 'norm'（不区分大小写）
    if 'whsp' in base_name.lower():
        normal_base = base_name.lower().replace('whsp', 'nature')
    else:
        # 如果没有whsp，尝试其他可能的模式
        print(f"Warning: 'whsp' not found in filename {filename}, trying alternative matching")
        normal_base = base_name.lower().replace('whisper', 'nature')
    
    # 恢复原始扩展名的大小写
    normal_filename = normal_base + ext
    
    # 构建normal文件的完整路径
    # 如果有子目录，保持相同的目录结构
    if rel_path.parent != Path('.'):
        normal_file = normal_path / rel_path.parent / normal_filename
    else:
        normal_file = normal_path / normal_filename
...
for whisper_file in tqdm(whisper_files, desc="Processing audio files"):
    try:
        ...
        if not normal_file.exists():
            print(f"Warning: Normal file does not exist: {normal_file} (for whisper: {whisper_file}), skipping")
            continue
